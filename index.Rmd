...
- Which county has the highest land area?
...
summary_info$avg_total_pop <- raw.data %>% 
  filter(year == 2002) %>%
  summarize(mean(total_pop))
summary_info$avg_total_pop #The average of total population across all counties in 2002

summary_info$King_highest_female_pop_yr <- raw.data %>% 
  filter(fips == "53033") %>% 
  filter(female_pop_15to64 == max(female_pop_15to64)) %>% 
  pull(year)
summary_info$King_highest_female_pop_yr #The year that King County, WA has the highest female population from 15 to 64

summary_info$King_highest_male_pop_yr <- raw.data %>% 
  filter(fips == "53033") %>% 
  filter(male_pop_15to64 == max(male_pop_15to64)) %>% 
  pull(year)
summary_info$King_highest_male_pop_yr #The year that King County, WA has the highest male population from 15 to 64

summary_info$Lowest_jail_pop_2018 <-raw.data %>% 
  filter(year == 2018) %>% 
  filter(total_jail_pop == min(total_jail_pop, na.rm = TRUE)) %>% 
  pull(county_name)
summary_info$Lowest_jail_pop_2018  #The county that has the largest land area

summary_info$Highest_jail_pop_2018 <-raw.data %>% 
  filter(year == 2018) %>% 
  filter(total_jail_pop == max(total_jail_pop, na.rm = TRUE)) %>% 
  pull(county_name)
summary_info$Highest_jail_pop_2018 #The county that has the largest jail population in 2018
```

The data includes columns like total population, total female population, total male population, etc. There are five values that might be valuable in analyzing the data set. First, across all years and counties, the average value of the total populations across counties is  `r summary_info$avg_total_pop `. When, analyzing specifically King County, there are also some valuable numbers. In the year `r summary_info$King_highest_female_pop_yr`, King county has the highest number of female aged from 15 to 64. In the year `r summary_info$King_highest_male_pop_yr`, King county has the highest number of male aged from 15 to 64. In 2018, among all the counties, `r summary_info$Lowest_jail_pop_2018` are found to have the lowest jail population. Also, it is found that in 2018, `r summary_info$Highest_jail_pop_2018` has the highest jail population.
...
ggplot(five_states) +
  ...
  labs(title = "Total population of different states in different years", x = "Year", y = "Total Population")
```
